...
diff_match() {
  ...
  if [[ $file =~ $target_file_pattern ]]; then
    ...
    docker run --rm -v "$left_temp:/diff/left" -v "$right_temp:/diff/right" -p 4567:4567 rozelin/gumtree:latest axmldiff left/$file_path right/$file_path > "$map_file_path.diff.xml"
